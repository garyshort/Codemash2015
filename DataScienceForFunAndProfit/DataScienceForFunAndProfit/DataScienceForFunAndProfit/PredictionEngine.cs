...
namespace DataScienceForFunAndProfit
{
    public class PredictionEngine
    {
        ...
        {
            var probabilites = new List<Probability>();
        ...
        public List<Probability> ConditionedOnMileTimes()
        {
            ...
            this.runnersAndRiders.ForEach(each =>
            {
                ...
                double sd = this.StandardDeviation(each.MileTimes);

                double correctedAverage = this.StandardDeviationTooBig(av, sd)
                        ? 0
                        : av;

                horseMileTimeAverages.Add(each, correctedAverage);
            });
            ...
            foreach (Horse h in horseMileTimeAverages.Keys.ToList())
            {
                ...
                var old = horseMileTimeAverages[h];
                horseMileTimeAverages[h] = largest / old;
            }

            // Normalise the averages to 1 by dividing each by their sum
            ...
            var probs = new List<Probability>();
        ...
        private double StandardDeviation(List<double> doubles)
        {
            return Math.Sqrt(
                doubles.Select(x =>
                    Math.Pow(x - doubles.Average(), 2)).Average());
        }
        ...
        {
            // Score each of the horses on their wins
            Dictionary<Horse, double> scores = new Dictionary<Horse, double>();

            // Score 3 for first place, 2 for second place and 1 for third place
            const double first = 3d;
            const double second = 2d;
            const double third = 1d;

            // Create a weighted frequency distribution for the 
            // horse's top three finishes
            this.runnersAndRiders.ForEach(horse =>
            {
                double score = 0;
                score += horse.Finishes.FindAll(each => each == 1).Sum() * first;
                score += horse.Finishes.FindAll(each => each == 2).Sum() * second;
                score += horse.Finishes.FindAll(each => each == 3).Sum() * third;
                scores[horse] = score;
            });

            // Normalise the scores
            double sum = scores.Values.Sum();
            scores.Keys.ToList().ForEach(horse => scores[horse] /= sum);

            // Return the probabilities
            List<Probability> probs = new List<Probability>();
            foreach (KeyValuePair<Horse, double> kvp in scores)
            {
                probs.Add(new Probability
                {
                    Horse = kvp.Key,
        ...
        public List<Probability> BayesProbWinningFromStall6()
        {
            var data = new
            {
                ...
                NotStall6Win = (double)45 / 100,
                NotStall6NotWin = (double)35 / 100
            };
            ...
            double probStall6GivenWin = data.Stall6 * data.stall6Win;
            double probWin = (data.Stall6 * data.stall6Win) +
                (data.NotStall6 * data.NotStall6Win
                );

            double probStall6 = data.Stall6;
            double probWinningGivenStall6 =
                probStall6GivenWin * probWin / probStall6;
